	...
	public static void initialize() {
		...
		dispatcher.register(literal(Commercialize.MOD_ID)
				...
				.then(literal("queryValue").then(argument("id", StringArgumentType.string()).executes(context -> {
					...
					if (itemId == null) {
						context.getSource().sendError(Text.literal("Invalid item identifier: '" + rawItemId + "'."));
						return 0;
					}
					...
					if (value == 0) {
						context.getSource().sendError(Text.literal("No value registered for item: '" + itemId + "'."));
						return 0;
					}

					context.getSource().sendFeedback(() -> Text.literal("Value for " + itemId + ": " + value), false);
				...
				.then(literal("compileAndSendMarketReports").requires(source -> source.hasPermissionLevel(4)).executes(context -> {
					...
					context.getSource().sendFeedback(() -> Text.literal("Compiled and sent "
							+ MarketAnalyticsUtil.getNumberOfReportsSentInLastTick() + " market report(s) to players."), true);
					return 1;
				}))
				...
				.then(literal("clearMailQueue").requires(source -> source.hasPermissionLevel(4))
						.then(argument("player", EntityArgumentType.player()).executes(context -> {
							...
							var pendingTransitItems = Commercialize.MAIL_TRANSIT_MANAGER.getItems().filter(transitItem -> {
								return transitItem.recipient.equals(player.getUuid());
							});
